...
import man from '../../assets/babra.jpg'
  ...
  <div className={classes.container}>
    <div className={classes.wrapper}>
      ...
      <div className={classes.right}>
        ...
        <img alt='' src={man} className={classes.profileUserImg} onClick={() => setShowModal(prev => !prev)} />
        {showModal &&
          ...
          {showMobileNav &&
            <div className={classes.navigation}>
              <div className={classes.left} onClick={() => setShowMobileNav(false)}>
                <Link  to='/'> <img className={classes.icon} src={icon} alt="" /></Link>
              </div>
                  ...
                  <div onClick={() => setSearchText("")} className={classes.allUsersContainer}>
                    {filteredUsers?.map((user) => (
                      <Link to={`/profileDetail/${user._id}`} key={user._id} onClick={() => setShowMobileNav(false)}>
                        <img alt='' src={user?.photo ? `http://localhost:5000/images/${user.photo}` : man} />
                        <div className={classes.userData}>
                          <span>{user?.username}</span>
                          <span>{user?.bio?.slice(0, 10)}</span>
                        </div>
